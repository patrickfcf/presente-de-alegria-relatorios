...
export function SharePage({ path, title }: { path: string; title: string }) {
  ...
  return <div className="share-page"><button className="text-button" onClick={async () => {
    ...
    try {
      ...
      if (!(error instanceof Error && error.name === "AbortError")) setNotice("Não foi possível compartilhar. Copie o endereço da página.");
    }
  }}>Compartilhar página</button><span className="small" role="status">{notice}</span></div>;
}
