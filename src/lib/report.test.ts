...
describe('attendance', () => {
  it('keeps unknown distinct from zero', () => {
    ...
    expect(attendanceTotal([null, null, null])).toEqual({ value: null, complete: false })
  })
  it('calculates complete and partial totals explicitly', () => {
    expect(attendanceTotal([12, 3, 5])).toEqual({ value: 20, complete: true })
    expect(attendanceTotal([12, null, 5])).toEqual({ value: 17, complete: false })
  })
  it.each(['-1', '1.5', 'NaN', '1e3', '1000001'])('rejects invalid quantity %s', value => {
