...
const socialImage =
  siteOrigin +
  manifest.icons.find((icon) => icon.sizes === "512x512" && icon.purpose === "any")
    .src;
...
const links = publicPages
  .map((p) => `<a href="${publicPath(p.path)}">${escape(p.heading)}</a>`)
  .join(" · ");
...
<script type="application/ld+json">${schema}</script></head>`,
  ...
  html = html.replace(
    '<div id="root"></div>',
    `<div id="root"><main><img src="/logo.png" width="157" alt="Presente de Alegria" /><h1>${escape(page.heading)}</h1><p>${escape(page.description)}</p><nav aria-label="Páginas públicas">${links}</nav><p><a href="https://presentedealegria.org.br/voluntario/">Quero ser voluntário</a> · <a href="/#/minha-celula">Minha célula</a></p><noscript>Ative o JavaScript para consultar as publicações atualizadas e usar o aplicativo.</noscript></main></div>`,
  );
...
await writeFile(
  "dist/sitemap.xml",
  `<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">${publicPages.map((p) => `<url><loc>${siteOrigin + publicPath(p.path)}</loc></url>`).join("")}</urlset>`,
);
...
await writeFile(
  "dist/404.html",
  '<!doctype html><html lang="pt-BR"><meta charset="UTF-8"><meta name="robots" content="noindex"><title>Página não encontrada | Presente de Alegria</title><h1>Página não encontrada</h1><p><a href="/sobre/">Conheça o Presente de Alegria</a></p></html>',
);
console.log(
  "Generated public SEO pages, sitemap, social metadata and crawler directives.",
);
