...
Páginas públicas canônicas: `/sobre`, `/eventos`, `/noticias`, `/ajudas`, `/instalar`. Endereços antigos com hash continuam abrindo a página equivalente. O aplicativo mantém a entrada privada em `/` e as rotas internas por hash, sem indexação.
...
`/sitemap.xml` inclui somente páginas públicas; `/robots.txt` permite rastrear assets. `noindex` no HTML de entrada privada e cabeçalhos dos domínios Pages evitam indexação duplicada. URLs inexistentes retornam a página 404 do Pages. RLS continua responsável pela segurança; robots não controla acesso.
...
- Ajudas: finalidade, regras, prazo, responsável e link oficial. Atualizar/arquivar campanhas encerradas; não inventar resultados ou depoimentos.
