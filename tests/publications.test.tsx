...
it("distinguishes network failure from an empty feed and supports retry", async () => {
  ...
  expect(await screen.findByText("Novas formas de ajudar em breve.")).toBeVisible();
});
it("hides draft and scheduled campaigns and removes participation from ended campaigns", () => {
  ...
  expect(screen.getAllByRole("button", { name: "Copiar chave Pix" })).toHaveLength(
    1,
  );
});
...
it("sorts events by publication and filters category, month and accent-insensitive location", () => {
  ...
  fireEvent.change(screen.getByLabelText("Mês do evento"), {
    target: { value: "2099-11" },
  });
  expect(screen.getAllByRole("article")).toHaveLength(1);
  fireEvent.change(screen.getByLabelText("Categoria"), {
    target: { value: "Formação" },
  });
    ...
    <NewsFeed
      news={[
        ...
        { ...base, id: "b", title: "Nova", published_at: "2020-02-01T12:00:00Z" },
        {
          ...
          id: "c",
          title: "Agendada",
          published_at: "2099-02-01T12:00:00Z",
        },
      ]}
    />,
  );
  expect(screen.getAllByRole("article")[0]).toHaveTextContent("Nova");
  fireEvent.change(screen.getByLabelText("Mês da publicação"), {
...
it("prefills event WhatsApp links using the Brasília date and preserves the contact", async () => {
  ...
  const result = new URL(
    eventContactUrl("https://wa.me/5511000000000?text=antigo", event),
  );
  expect(result.pathname).toBe("/5511000000000");
  expect(result.searchParams.get("text")).toBe(
    "Olá! Vi o evento “Encontro & Alegria”, do dia 17/10/2026, no aplicativo do Presente de Alegria e gostaria de saber mais informações sobre como participar. Pode me ajudar?",
  );
  const api = new URL(
    eventContactUrl("https://api.whatsapp.com/send?phone=5511000000000", event),
  );
  expect(api.searchParams.get("phone")).toBe("5511000000000");
  expect(api.searchParams.get("text")).toBe(result.searchParams.get("text"));
  for (const url of [
    "https://example.org/contato",
    "https://chat.whatsapp.com/exemplo",
    "https://wa.me.evil.invalid/5511000000000",
  ]) {
