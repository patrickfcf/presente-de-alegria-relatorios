...
vi.mock("../src/lib/api", () => ({ configured: true, supabase: { auth: { getSession: vi.fn().mockResolvedValue({data:{session:null}}), onAuthStateChange: vi.fn().mockReturnValue({data:{subscription:{unsubscribe:vi.fn()}}}) } }, rows: vi.fn(), admin: vi.fn(), publicRows: vi.fn().mockResolvedValue([]) }));
...
it("opens the clean public events URL with crawlable navigation before login", async () => {
  ...
  expect(screen.getByRole("link", {name: "Minha célula", exact: true})).toHaveAttribute("href", "/#/minha-celula");
});
...
it("introduces four areas at the root without adding a home menu item", () => {
  render(<App />);
  expect(screen.getByRole("heading", {name: "Alegria que aproxima."})).toBeVisible();
  expect(screen.getByRole("link", {name: /Acessar minha célula/})).toHaveAttribute("href", "/#/minha-celula");
  const menu = within(screen.getByRole("navigation", {name: "Menu principal"}));
  expect(menu.getAllByRole("link")).toHaveLength(4);
  expect(menu.getByRole("link", {name: "Doação"})).toHaveAttribute("href", "/ajudas/");
...
it("keeps the cell entry behind login", async () => {
  ...
  render(<App />);
  expect(await screen.findByRole("heading", {name: "Quem é você no Presente?"})).toBeVisible();
});
